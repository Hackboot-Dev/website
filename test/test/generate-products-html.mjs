...
// Load DATABASE_URL from repo root .env if not set
if (!process.env.DATABASE_URL) {
  try {
    ...
    // Fallback: try .env.example if .env not present
    if (!process.env.DATABASE_URL) {
...
try {
  ...
  console.error('Erreur: @prisma/client introuvable. Installez les dépendances.');
...
function render(products) {
  const items = products.map((p) => {
    ...
    const instances = (p.instances || []).map(i => `
      <tr>
        <td class="py-1 pr-3"><code class="mono">${i.id}</code></td>
        <td class="py-1 pr-3"><span class="badge">${i.status}</span></td>
        ...
        <td class="py-1 text-zinc-400">${escapeHtml(new Date(i.createdAt).toLocaleString())}</td>
      </tr>
    `).join('');
    return `
      <section class="card p-6 md:p-8">
        <div class="flex items-start justify-between gap-6">
          ...
          <div class="text-right text-sm text-zinc-400">
            ...
            <div class="mt-1">Instances liées: ${p._count?.instances ?? 0}</div>
            <div class="mt-1">Créé: ${escapeHtml(new Date(p.createdAt).toLocaleString())}</div>
            <div class="">MAJ: ${escapeHtml(new Date(p.updatedAt).toLocaleString())}</div>
          </div>
        </div>
        ...
        <div class="mt-6">
          <h3 class="text-sm text-zinc-300 mb-2">Instances (récentes d'abord)</h3>
  ...
  return layout({
    title: 'Listing Produits — VMCloud',
    body: `<main class="container mx-auto px-6 py-10 space-y-6">${items || '<div class=\'text-zinc-400\'>Aucun produit trouvé.</div>'}</main>`
  });
}
    ...
    const products = await prisma.product.findMany({
      ...
      include: {
        ...
        instances: {
          ...
          select: { id: true, status: true, createdAt: true, userId: true, name: true, hostname: true }
        }
      }
    });
