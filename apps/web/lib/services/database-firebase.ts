...
export class DatabaseService {
  ...
  // ============================================================
  // AGGREGATED CLIENT STATS (optimized for dashboard)
  // ============================================================

  async getAggregatedClientStats(): Promise<AggregatedClientStats | null> {
    const db = this.getDb();
    const docRef = doc(db, 'stats', 'clients');
    const docSnap = await getDoc(docRef);
    return docSnap.exists() ? (docSnap.data() as AggregatedClientStats) : null;
  }
  ...
  async refreshAggregatedClientStats(): Promise<AggregatedClientStats> {
    ...
    // Get all clients (one-time load for full refresh)
    const clients = await this.getClients();
    ...
    const business = clients.filter(c => c.type === 'business' || c.type === 'enterprise').length;
    ...
    // Find top client
    const topClientData = clients.reduce((top, c) =>
      (!top || (c.totalRevenue || 0) > (top.totalRevenue || 0)) ? c : top
    , null as Client | null);

    const stats: AggregatedClientStats = {
      total,
      active,
      inactive,
      leads,
      ...
      topClient: topClientData ? {
        id: topClientData.id,
        name: topClientData.name,
        email: topClientData.email,
        type: topClientData.type,
        totalRevenue: topClientData.totalRevenue || 0,
        totalTransactions: topClientData.totalTransactions || 0,
      } : null,
      updatedAt: getCurrentTimestamp(),
      lastFullRefreshAt: getCurrentTimestamp(),
    };

    // Save to Firestore
    await setDoc(doc(db, 'stats', 'clients'), stats);

    return stats;
  }

  async incrementClientStats(change: {
    total?: number;
    active?: number;
    inactive?: number;
    leads?: number;
    ...
    const db = this.getDb();
    const docRef = doc(db, 'stats', 'clients');
    const existing = await this.getAggregatedClientStats();

    if (!existing) {
      // No stats yet, do full refresh
      await this.refreshAggregatedClientStats();
      return;
    }

    // Incremental update
    const updated: Partial<AggregatedClientStats> = {
      total: Math.max(0, (existing.total || 0) + (change.total || 0)),
      active: Math.max(0, (existing.active || 0) + (change.active || 0)),
      inactive: Math.max(0, (existing.inactive || 0) + (change.inactive || 0)),
      leads: Math.max(0, (existing.leads || 0) + (change.leads || 0)),
      churned: Math.max(0, (existing.churned || 0) + (change.churned || 0)),
      business: Math.max(0, (existing.business || 0) + (change.business || 0)),
      individual: Math.max(0, (existing.individual || 0) + (change.individual || 0)),
      newThisMonth: Math.max(0, (existing.newThisMonth || 0) + (change.newThisMonth || 0)),
      avgRevenue: (existing.total || 0) + (change.total || 0) > 0
        ? existing.totalRevenue / ((existing.total || 0) + (change.total || 0))
        : 0,
      updatedAt: getCurrentTimestamp(),
    };

    await updateDoc(docRef, updated as DocumentData);
  }

  async searchClients(searchQuery: string, limitCount: number = 10): Promise<Client[]> {
    // For search, we need to query Firestore efficiently
    // Since Firestore doesn't support full-text search, we use client-side filtering
    // but only load what we need using pagination
    const db = this.getDb();
    const q = query(
      collection(db, 'clients'),
      orderBy('name'),
      limit(100) // Load max 100 for search
    );
    const snapshot = await getDocs(q);
