...
// /workspaces/website/apps/web/app/legal/changes/page.tsx
// Description: Changes Policy page (FR/EN) rendered from Markdown with rich styling
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const resp = await fetch(`/data/legal/changes/${lang}.md`, { cache: 'no-store' });
        if (!resp.ok) {
          // fallback to FR
          const fr = await fetch('/data/legal/changes/fr.md', { cache: 'no-store' });
          if (!fr.ok) throw new Error('Unable to load changes policy');
        ...
        setError(e?.message || 'Failed to load changes policy');
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-5xl mx-auto">
            ...
            <h1 className="text-3xl sm:text-4xl font-extralight text-white mb-4">
              {language === 'en' ? 'Changes and Deprecations Policy' : 'Politique de changements et dépréciations'}
            </h1>
            <p className="text-zinc-500 mb-10">
              {language === 'en'
                ? 'How we manage service changes, deprecations, and end-of-life procedures. Migration assistance and client rights.'
                : 'Comment nous gérons les changements de service, les dépréciations et les procédures de fin de vie. Assistance à la migration et droits des clients.'}
            </p>
