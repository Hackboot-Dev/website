...
- /apps/web/app/[locale]/admin/AdminDashboardClient.tsx
- /apps/web/app/[locale]/admin/clients/ClientsPageClient.tsx

CHANGEMENTS:
```
1. LAYOUT ADMIN (layout.tsx)
   - Header compact sur mobile (h-14 au lieu de h-16)
   - Bottom navigation fixe style app (5 onglets)
   - Menu hamburger avec overlay slide-in depuis la droite
   - Safe-area support pour iPhone (notch)
   - Navigation desktop conservée avec dropdowns animés
   - Padding adaptatif (px-4 mobile, px-6+ desktop)
   - Main content avec pb-20 pour la bottom nav

2. DASHBOARD (AdminDashboardClient.tsx)
   - Espacements réduits sur mobile (space-y-6 vs space-y-12)
   - Titres adaptatifs (text-2xl mobile → text-5xl desktop)
   - Stats grid en 2 colonnes sur mobile (au lieu de 4)
   - Cards avec padding réduit (p-3 mobile, p-6 desktop)
   - Coins arrondis sur mobile (rounded-xl)
   - Active:scale feedback tactile

3. PAGE CLIENTS (ClientsPageClient.tsx)
   - StatCard responsive (taille réduite sur mobile)
   - Barre de recherche pleine largeur sur mobile
   - Filtres scrollables horizontalement
   - Boutons avec feedback tactile (active:scale-95)
   - Modal style bottom-sheet sur mobile (slide-up)
   - Boutons d'action en colonne sur mobile
   - Sticky header et footer dans le modal
```

PROCHAINE ÉTAPE: Test visuel sur différents appareils mobiles
---
